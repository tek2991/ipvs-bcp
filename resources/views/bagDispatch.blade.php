<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('bag-dispatch.store') }}" method="post">
                    @csrf
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 bg-white border-b border-gray-200">
    ...
    @if ($bags_dispatched->total() > 0)
        <div class="py-6">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6">
                    {{ $bags_dispatched->links() }}
                </div>
            </div>
        </div>
        ...
        <div class="py-6">
            ...
            @foreach ($bags_dispatched as $bag_dispatched)
                <div class="p-1">
                    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <div class="p-4 bg-white border-b border-gray-200">
                            <div class="grid grid-cols-5 gap-1">
                                <div class="text-lg font-semibold">{{ $bag_dispatched->bag_no }}</div>
                                <span>{{ $bag_dispatched->bagType->description }}({{ $bag_dispatched->bagTransactionType->description }})</span>
                                <span>{{ $bag_dispatched->toFacility->name }}</span>
                                <span>{{ $bag_dispatched->creator->name }}</span>
                                <span>{{ $bag_dispatched->created_at }}</span>
                            </div>
                        </div>
                    </div>
                </div>
