                    ...
                    <form method="POST" action="{{ route('facility.update', $facility->id) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-input id="facility_code" class="block mt-1 w-full" type="text"
                                    name="facility_code" required value="{{ $facility->facility_code }}" />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <x-input-select id="facility_type_id" class="block mt-1 w-full" name="facility_type_id"
                                    required>
                                    @foreach ($facility_types as $facility_type)
                                        <option value="{{ $facility_type->id }}"
                                            {{ $facility_type->id == $facility->facility_type_id ? 'selected' : '' }}>
                                            {{ $facility_type->name }}
                                        </option>
                                    @endforeach
                                </x-input-select>
                                ...
                                <x-input-select id="district_id" class="block mt-1 w-full" name="district_id" required>
                                    @foreach ($districts as $district)
                                        <option value="{{ $district->id }}"
                                            {{ $district->id == $facility->district_id ? 'selected' : '' }}>
                                            {{ $district->name }}
                                        </option>
                                    @endforeach
                                </x-input-select>
                            ...
                            <div>
                                ...
                                <x-input-select id="reporting_circle_id" class="block mt-1 w-full"
                                    name="reporting_circle_id" required>
                                    @foreach ($reporting_circles as $reporting_circle)
                                        <option value="{{ $reporting_circle->id }}"
                                            {{ $reporting_circle->id == $facility->reporting_circle_id ? 'selected' : '' }}>
