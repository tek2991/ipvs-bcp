                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-input-select id="facility_id" class="block mt-1 w-full" name="facility_id" required>
                                    @foreach ($active_facilities as $facility)
                                        <option value="{{ $facility->id }}" {{ $facility->id == $user->facility_id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <x-input-select id="cadre_id" class="block mt-1 w-full" name="cadre_id" required>
                                    @foreach ($cadres as $cadre)
                                        <option value="{{ $cadre->id }}" {{ $cadre->id == $user->cadre_id ? 'selected' : '' }} >{{ $cadre->name }}</option>
                                    @endforeach
                                </x-input-select>
                            </div>
                            ...
                            <div>
                                ...
                                <x-input id="password" class="block mt-1 w-full" type="password" name="password"
                                    placeholder="Leave empty if no changes required!" />
                            </div>
                            <div>
                                ...
                                <x-input id="confirm_password" class="block mt-1 w-full" type="password"
                                    name="password_confirmation" placeholder="Leave empty if no changes required!" />
                            </div>
                        </div>
